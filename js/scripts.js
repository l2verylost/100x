...
function onScroll(event){
	...
	$(".menu a[href*=#]").each(function () {
        ...
        if(currLink){
            ...
            if (refElement && refElement.position().top -50 <= scrollPos && refElement.position().top + refElement.height() -50 > scrollPos) {
...
$(document).ready(function() {
        $(".fancybox").fancybox();
        fetch("/ftp/online.txt").then(res=>res.text()).then(res=>{
            document.getElementById("onlinecount").innerHTML = res;
        })
	});